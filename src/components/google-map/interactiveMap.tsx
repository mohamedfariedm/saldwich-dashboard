...
const TableFilter = dynamic(
  ...
  export default function GoogleMap({text_tr, data}: {text_tr?:any,data:any|null}) {
      ...
      <div className='flex flex-col gap-3'>
          ...
          <div className='flex gap-3'>
              <div className='flex gap-1 items-center'>
                  ...
                  <div>{text_tr.employee_places.Promoter} <span>( {text_tr.employee_places.Active}: {data?.active_promoters_count} / {text_tr.employee_places.All} : {data?.promoters_count})</span></div>
              </div>
              <div className='flex gap-1 items-center'>
                  <div className='w-2 h-2 rounded' style={{backgroundColor: 'blue'}}>{''}</div>
                  <div>{text_tr.employee_places.Marchindaizer} <span>( {text_tr.employee_places.Active}: {data?.active_Marchindaizers_count} / {text_tr.employee_places.All} : {data?.Marchindaizers_count} )</span></div>
              </div>
              <div className='flex gap-1 items-center'>
                  <div className='w-2 h-2 rounded' style={{backgroundColor: 'yellow'}}>{''}</div>
                  <div>{text_tr.employee_places.Supervisor}<span>( {text_tr.employee_places.Active}: {data?.active_Supervisors_count} / {text_tr.employee_places.All} : {data?.Supervisors_count} )</span></div>
              </div>
          </div>
