...
import {
  CategoryFormInput,
  CategoryFormSchema,
} from '@/utils/validators/categor-form.schema';
import { useCreateCategory, useUpdateCategory } from '@/framework/category';
...
export default function UpdateCreateBrand({
  ...
  const [show, setshow] = useState<boolean>(
    initValues ? initValues.show_in_homepage : true
  );
    ...
    if (!isImageData) {
      ...
      if (initValues) {
        ...
        update({
          ...
          products: data.products.map((brand) => brand.value),
          active: active,
          show_in_homepage: show,
      ...
      <Form<CategoryFormInput>
        ...
        useFormProps={{
          defaultValues: {
            ...
            products: initValues
              ? initValues?.products?.map((brand: any) => {
            ...
            <>
              ...
              >
                    ...
                    <CreatableSelect
                      styles={{
                        control: (baseStyles, state) => ({
                          ...
                          boxShadow: '',
                          borderColor: `${
                            errors?.products?.message ? 'red' : 'none'
                          }`,
                        }),
                      }}
                ...
                <span className="rizzui-input-error-text mt-0.5 text-xs text-red">
                  {errors?.products?.message ? 'is Required' : ''}
                </span>
              </FormGroup>
              ...
              <div className="flex items-center justify-end gap-4">
                ...
                <Button
                  ...
                  className="w-full @xl:w-auto"
                >
                  {initValues ? 'Update Categiey' : 'Create Categiey'}
                </Button>
              </div>
            </>
          );
