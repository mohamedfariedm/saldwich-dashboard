...
export default function InqueriesTable({        text_tr,
  permitions,  data = [], getSelectedColumns, getSelectedRowKeys, totalItems }: {text_tr?:any,  permitions?: string[],  data: any[], getSelectedColumns: React.Dispatch<React.SetStateAction<any[]>>, getSelectedRowKeys: React.Dispatch<React.SetStateAction<any[]>>, totalItems: number }) {
  const { mutate: deleteRegion } = useDeleteJourney();

  const searchParams = useSearchParams()
  const params = new URLSearchParams(searchParams)
  const [pageSize, setPageSize] = useState(Number(params.get('limit')));
  const { data: allUsers } = useUsersByRole(['Supervisor', 'Marchindaizer',"Promoter"]);
