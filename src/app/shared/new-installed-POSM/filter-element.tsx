...
//   brandFilter,
  ...
  return (
    <>
      ...
      <div className='flex gap-2'>
        <DateFiled
            selected={filters['remove_date_from'] ? new Date(filters['remove_date_from']) : null}
            onChange={(date: any) => {
            updateFilter('remove_date_from', formatDate(date, 'YYYY-MM-DD'));
            }}
            placeholderText="Remove date from"
            {...(isMediumScreen && {
            inputProps: {
                label: 'Remove Date From',
                labelClassName: 'font-medium text-gray-700',
            },
            })}
        />
        <DateFiled
            selected={filters['remove_date_to'] ? new Date(filters['remove_date_to']) : null}
            onChange={(date: any) => {
            updateFilter('remove_date_to', formatDate(new Date(date), 'YYYY-MM-DD'));
            }}
            placeholderText="Remove date to"
            {...(isMediumScreen && {
            inputProps: {
                label: 'Remove Date to',
                labelClassName: 'font-medium text-gray-700',
            },
            })}
        />
      ...
      <div className='flex gap-2'>
        <DateFiled
            selected={filters['gondolaStatusDateFrom_from'] ? new Date(filters['gondolaStatusDateFrom_from']) : null}
            onChange={(date: any) => {
            updateFilter('gondolaStatusDateFrom_from', formatDate(date, 'YYYY-MM-DD'));
            }}
            placeholderText="Select start date"
            {...(isMediumScreen && {
            inputProps: {
                label: 'Start Gondola Date from',
                labelClassName: 'font-medium text-gray-700',
            },
            })}
        />
        <DateFiled
            selected={filters['gondolaStatusDateFrom_to'] ? new Date(filters['gondolaStatusDateFrom_to']) : null}
            onChange={(date: any) => {
            updateFilter('gondolaStatusDateFrom_to', formatDate(date, 'YYYY-MM-DD'));
            }}
            placeholderText="Select start date"
            {...(isMediumScreen && {
            inputProps: {
                label: 'Start Gondola Date to',
                labelClassName: 'font-medium text-gray-700',
            },
            })}
        />
      ...
      <div className='flex gap-2'>
        <DateFiled
            selected={filters['gondolaStatusDateTo_from'] ? new Date(filters['gondolaStatusDateTo_from']) : null}
            onChange={(date: any) => {
            updateFilter('gondolaStatusDateTo_from', formatDate(date, 'YYYY-MM-DD'));
            }}
            placeholderText="Select End date"
            {...(isMediumScreen && {
            inputProps: {
                label: 'End Gondola Date from',
                labelClassName: 'font-medium text-gray-700',
            },
            })}
        />
        <DateFiled
            selected={filters['gondolaStatusDateTo_to'] ? new Date(filters['gondolaStatusDateTo_to']) : null}
            onChange={(date: any) => {
            updateFilter('gondolaStatusDateTo_to', formatDate(date, 'YYYY-MM-DD'));
            }}
            placeholderText="Select End date"
            {...(isMediumScreen && {
            inputProps: {
                label: 'End Gondola Date to',
                labelClassName: 'font-medium text-gray-700',
            },
            })}
        />
