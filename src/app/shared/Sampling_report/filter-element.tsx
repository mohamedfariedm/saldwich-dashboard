  ...
  return (
    <>
      <div className='flex gap-2'>
        <DateFiled
            selected={filters['samplingDateFrom_from'] ? new Date(filters['samplingDateFrom_from']) : null}
            onChange={(date: any) => {
            updateFilter('samplingDateFrom_from', formatDate(date, 'YYYY-MM-DD'));
            }}
            placeholderText="Select start date"
            {...(isMediumScreen && {
            inputProps: {
                label: 'Start Sampling Date from',
                labelClassName: 'font-medium text-gray-700',
            },
            })}
        />
        <DateFiled
            selected={filters['samplingDateFrom_to'] ? new Date(filters['samplingDateFrom_to']) : null}
            onChange={(date: any) => {
            updateFilter('samplingDateFrom_to', formatDate(date, 'YYYY-MM-DD'));
            }}
            placeholderText="Select start date"
            {...(isMediumScreen && {
            inputProps: {
                label: 'Start Sampling Date to',
                labelClassName: 'font-medium text-gray-700',
            },
            })}
        />
      ...
      <div className='flex gap-2'>
        <DateFiled
            selected={filters['samplingDateTo_from'] ? new Date(filters['samplingDateTo_from']) : null}
            onChange={(date: any) => {
            updateFilter('samplingDateTo_from', formatDate(date, 'YYYY-MM-DD'));
            }}
            placeholderText="Select End date"
            {...(isMediumScreen && {
            inputProps: {
                label: 'End Sampling Date from',
                labelClassName: 'font-medium text-gray-700',
            },
            })}
        />
        <DateFiled
            selected={filters['samplingDateTo_to'] ? new Date(filters['samplingDateTo_to']) : null}
            onChange={(date: any) => {
            updateFilter('samplingDateTo_to', formatDate(date, 'YYYY-MM-DD'));
            }}
            placeholderText="Select End date"
            {...(isMediumScreen && {
            inputProps: {
                label: 'End Sampling Date to',
                labelClassName: 'font-medium text-gray-700',
            },
            })}
        />
