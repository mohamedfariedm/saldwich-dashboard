...
import { PiCalendarBlank, PiCaretDownBold, PiTrashDuotone } from 'react-icons/pi';
...
const [selectedSubSubCat, setSelecedSubSubCat] = useState<{name: string, id: string}[]>(seletedSubCategory?.find(((cat: any) => cat?.id == filters['VCP_id']))?.children||[])
  ...
  console.log(seletedSubCategory);
 ...
 />
      ...
      <div className='flex gap-2'>
        ...
        closeMenuOnSelect={true}
          ...
          value={filters['sku_code']?{label:productDetails?.find((option) => option.value === filters['sku_code'])?.label,value:filters['sku_code']}:""}
          styles={{
            control: (baseStyles, state) => ({
              ...baseStyles,
              boxShadow:"",
          ...
          onChange={(value: any) => {
            updateFilter('sku_code', value.value);
          }}
        />
        </div>
      </div>
