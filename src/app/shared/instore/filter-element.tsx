  ...
  return (
    <>
       <div className='flex gap-2'>
        <DateFiled
            selected={filters['inStoreDateFrom_from'] ? new Date(filters['inStoreDateFrom_from']) : null}
            onChange={(date: any) => {
            updateFilter('inStoreDateFrom_from', formatDate(date, 'YYYY-MM-DD'));
            }}
            placeholderText="Select start date "
            {...(isMediumScreen && {
            inputProps: {
                label: 'Start Instore Date from',
                labelClassName: 'font-medium text-gray-700',
            },
            })}
        />
        <DateFiled
            selected={filters['inStoreDateFrom_to'] ? new Date(filters['inStoreDateFrom_to']) : null}
            onChange={(date: any) => {
            updateFilter('inStoreDateFrom_to', formatDate(date, 'YYYY-MM-DD'));
            }}
            placeholderText="Select end date "
            {...(isMediumScreen && {
            inputProps: {
                label: 'Start Instore Date to',
                labelClassName: 'font-medium text-gray-700',
            },
            })}
        />
      ...
      <div className='flex gap-2'>
        <DateFiled
            selected={filters['inStoreDateTo_from'] ? new Date(filters['inStoreDateTo_from']) : null}
            onChange={(date: any) => {
            updateFilter('inStoreDateTo_from', formatDate(date, 'YYYY-MM-DD'));
            }}
            placeholderText="Select start date"
            {...(isMediumScreen && {
            inputProps: {
                label: 'End Instore Date from',
                labelClassName: 'font-medium text-gray-700',
            },
            })}
        />
        <DateFiled
            selected={filters['inStoreDateTo_to'] ? new Date(filters['inStoreDateTo_to']) : null}
            onChange={(date: any) => {
            updateFilter('inStoreDateTo_to', formatDate(date, 'YYYY-MM-DD'));
            }}
            placeholderText="Select End date"
            {...(isMediumScreen && {
            inputProps: {
                label: 'End Instore Date to',
                labelClassName: 'font-medium text-gray-700',
            },
            })}
        />
