...
//   brandFilter,
  ...
  return (
    ...
    <div className='flex gap-2'>
        <DateFiled
            selected={filters['installationDateFrom_from'] ? new Date(filters['installationDateFrom_from']) : null}
            onChange={(date: any) => {
            updateFilter('installationDateFrom_from', formatDate(date, 'YYYY-MM-DD'));
            }}
            placeholderText="Select start date"
            {...(isMediumScreen && {
            inputProps: {
                label: 'Start Installation from',
                labelClassName: 'font-medium text-gray-700',
            },
            })}
        />
        <DateFiled
            selected={filters['installationDateFrom_to'] ? new Date(filters['installationDateFrom_to']) : null}
            onChange={(date: any) => {
            updateFilter('installationDateFrom_to', formatDate(date, 'YYYY-MM-DD'));
            }}
            placeholderText="Select start date"
            {...(isMediumScreen && {
            inputProps: {
                label: 'Start Installation to',
                labelClassName: 'font-medium text-gray-700',
            },
            })}
        />
      ...
      <div className='flex gap-2'>
        <DateFiled
            selected={filters['installationDateTo_from'] ? new Date(filters['installationDateTo_from']) : null}
            onChange={(date: any) => {
            updateFilter('installationDateTo_from', formatDate(date, 'YYYY-MM-DD'));
            }}
            placeholderText="Select End date"
            {...(isMediumScreen && {
            inputProps: {
                label: 'End Installation from',
                labelClassName: 'font-medium text-gray-700',
            },
            })}
        />
        <DateFiled
            selected={filters['installationDateTo_to'] ? new Date(filters['installationDateTo_to']) : null}
            onChange={(date: any) => {
            updateFilter('installationDateTo_to', formatDate(date, 'YYYY-MM-DD'));
            }}
            placeholderText="Select End date"
            {...(isMediumScreen && {
            inputProps: {
                label: 'End Installation to',
                labelClassName: 'font-medium text-gray-700',
            },
            })}
        />
      ...
      <div className='w-full'>
        ...
        <div className='w-full'>
          <label className='font-medium text-gray-700'> Have Permanent Promoters ?</label>
        <CreatableSelect
        className="pt-2 w-full"
        closeMenuOnSelect={true}
          options={statusOptions}
          value={filters['have_permanent_promoters']?{label:statusOptions?.find((option) => option.value === filters['have_permanent_promoters'])?.label,value:filters['have_permanent_promoters']}:""}
          styles={{
            control: (baseStyles, state) => ({
              ...baseStyles,
              boxShadow:"",
          ...
          placeholder='Have Permanent Promoters ?'
          onChange={(value: any) => {
            updateFilter('have_permanent_promoters', value.value);
          }}
        />
